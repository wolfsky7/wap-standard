...
export const getUrlParam = (name, source) => {
	...
	var r = (source || window.location.href).match(reg);
...
export const object2Query = (body, willEncode = false) => {
	return Object.keys(body || {}).reduce((last, key) => {
		const ty = typeof body[key]
		if (ty == "function") {
			return last;
		}
		const v = ty == "object" ? JSON.stringify(body[key]) : (willEncode ? encodeURIComponent(body[key]) : body[key]);
		last += (last ? "&" : "") + key + "=" + (v);
		return last;
	}, "") || ""
	// return Object.keys(body || {}).reduce((last, key) => {
	// 	last += (last ? "&" : "") + key + "=" + encodeURIComponent(body[key]);
	// 	return last;
	// }, "") || ""
}
